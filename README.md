...
固定判定标准：20 个有效随机挑战；完整状态至少命中 15 次；删除 item ID 后至少命中 15 次；message-only、损坏密文和明文泄漏均必须为 0。

## 随机间隔持续监控

双击 `start_monitor.bat`，输入两个 API 后，间隔设置直接按回车即可。默认每轮完成后随机等待 20–40 秒。窗口保持开启，需要停止时按一次 `Ctrl+C`。

持续模式会滚动统计最近 20 个有效挑战，并沿用 `15/20 + 所有阴性对照为 0` 的严格门槛。最初不足 20 个有效挑战时显示 `warming_up`。详细说明见 [MONITORING_CN.md](MONITORING_CN.md)。
...
程序会发送标准的 API 客户端请求头。如果待测接口返回 403、429、503，或返回的不是 JSON，这说明请求被网关、Cloudflare 或限流系统拦住了。此类请求不会被当成“模型不是 GPT-5.6”，单次检测会给出结果不确定，持续监控会显示状态码并等待下一轮有效请求。
